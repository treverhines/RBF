...
def gmres_callback(rk,_iter=[0]):
    l2 = np.linalg.norm(rk)
    print('iteration %4d: GMRES residual norm: %s' % (_iter[0],l2))
    _iter[0] += 1
...
def build_system_matrix(n):
    ...
    G = scipy.sparse.vstack((D,dD_fix,dD_free)).tocsc()
    return G
...
print('building small system')
G_pre = build_system_matrix(n_pre)
plt.imshow(np.abs(G_pre.A) > 0.0)
plt.show()
print('performing LU decomposition')
lu = scipy.sparse.linalg.splu(G_pre)
M = scipy.sparse.linalg.LinearOperator(G_pre.shape,lu.solve) 

print('building large system')
G = build_system_matrix(n_full)

plt.imshow(np.abs(G.A) > 0.0)
plt.show()
print('solving with GMRES')
u,info = scipy.sparse.linalg.gmres(G,d,M=M,callback=gmres_callback)
print('exited with info: %s' % info)
# reshape the solution
u = np.reshape(u,(2,-1))
u_x,u_y = u
## Calculate strain from displacements
D_x = weight_matrix(nodes,nodes,(1,0),n=n_full)
D_y = weight_matrix(nodes,nodes,(0,1),n=n_full)
e_xx = D_x.dot(u_x)
e_yy = D_y.dot(u_y)
