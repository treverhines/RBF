...
# use the ghost nodes to enforce the body force constraints on the
# free boundary
out = elastic3d_body_force(nodes[idx['boundary:free']], nodes, lamb=lamb, mu=mu, n=n)
...
out = elastic3d_body_force(nodes[idx['boundary:roller']], nodes, lamb=lamb, mu=mu, n=n)
G_xx = add_rows(G_xx, out['xx'], idx['ghosts:roller'])
G_xy = add_rows(G_xy, out['xy'], idx['ghosts:roller'])
G_xz = add_rows(G_xz, out['xz'], idx['ghosts:roller'])
G_yx = add_rows(G_yx, out['yx'], idx['ghosts:roller'])
G_yy = add_rows(G_yy, out['yy'], idx['ghosts:roller'])
G_yz = add_rows(G_yz, out['yz'], idx['ghosts:roller'])
G_zx = add_rows(G_zx, out['zx'], idx['ghosts:roller'])
G_zy = add_rows(G_zy, out['zy'], idx['ghosts:roller'])
G_zz = add_rows(G_zz, out['zz'], idx['ghosts:roller'])

# add free surface constraints
out = elastic3d_surface_force(nodes[idx['boundary:free']], normals[idx['boundary:free']], nodes, lamb=lamb, mu=mu, n=n)
...
G_xz = add_rows(G_xz, normals_z.dot(out['zz']), idx['boundary:roller'])

# free parallel to the surface
out = elastic3d_surface_force(nodes[idx['boundary:roller']], normals[idx['boundary:roller']], nodes, lamb=lamb, mu=mu, n=n)
parallels_1,parallels_2 = find_orthogonals(normals[idx['boundary:roller']])

parallels_x = sp.diags(parallels_1[:, 0])
parallels_y = sp.diags(parallels_1[:, 1])
parallels_z = sp.diags(parallels_1[:, 2])
G_yx = add_rows(G_yx, parallels_x.dot(out['xx']) + parallels_y.dot(out['yx']) + parallels_z.dot(out['zx']), idx['boundary:roller'])
G_yy = add_rows(G_yy, parallels_x.dot(out['xy']) + parallels_y.dot(out['yy']) + parallels_z.dot(out['zy']), idx['boundary:roller'])
G_yz = add_rows(G_yz, parallels_x.dot(out['xz']) + parallels_y.dot(out['yz']) + parallels_z.dot(out['zz']), idx['boundary:roller'])

parallels_x = sp.diags(parallels_2[:, 0])
parallels_y = sp.diags(parallels_2[:, 1])
parallels_z = sp.diags(parallels_2[:, 2])
G_zx = add_rows(G_zx, parallels_x.dot(out['xx']) + parallels_y.dot(out['yx']) + parallels_z.dot(out['zx']), idx['boundary:roller'])
...
ax.plot(nodes[idx['boundary:free'], 0], nodes[idx['boundary:free'], 1], 'C0.', zorder=0)
# plot a contour map of the topography
CS = ax.tricontour(nodes[idx['boundary:free'], 0],
                   nodes[idx['boundary:free'], 1],
                   nodes[idx['boundary:free'], 2], 4, colors='k', zorder=1)
