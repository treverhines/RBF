...
class Test(unittest.TestCase):
  def test_run_mean(self):
    ...
    gp = rbf.gpr.PriorGaussianProcess(rbf.basis.ga,(1.0,2.0,3.0))  
    gp.mean([[0.0],[1.0],[2.0]],diff=(0,))
    return 
    
  def test_run_covariance(self):
    # make sure the covariance method runs without failure
    gp = rbf.gpr.PriorGaussianProcess(rbf.basis.ga,(0.0,1.0,1.0))  
  ...
  def test_run_add(self):
    ...
    gp1 = rbf.gpr.PriorGaussianProcess(rbf.basis.ga,(0.0,1.0,1.0))  
    gp2 = rbf.gpr.PriorGaussianProcess(rbf.basis.ga,(0.0,1.0,1.0))  
    out = gp1 + gp2
    return 
  ...
  def test_condition(self):  
    ...
    gp = rbf.gpr.PriorGaussianProcess(rbf.basis.ga,(0.0,2.0,0.5),order=-1)  
    n = 500
    m = 1000
    ...
    self.assertTrue(abs(mean_chi2 - 1.0) < 0.3)
    #plt.plot(x,mean,'b-')
