#!/usr/bin/env python
# this function demonstrates rbf.filter.filter for one-dimensional 
...
ylim = ax2.get_ylim()
ax2.set_ylim(ylim)
# plot the expected frequency content of the filtered solution
ax2.loglog(freq[1:],(1.0/(1.0 + (freq[1:]/cutoff)**(2*order)))**2,'k--')

ax1.grid(True)
ax1.set_xlabel(u'$\mathrm{time}$')
ax1.set_ylim((-7,7))
ax1.legend([u'$\mathrm{observed}$',u'$\mathrm{filtered},\ \omega_c=10^{-2}$'],frameon=False,fontsize=12,loc=0)
ax2.set_xlabel(u'$\mathrm{frequency}$')
ax2.set_ylabel(u'$\mathrm{power\ spectral\ density}$')
ax2.set_xlim((10**-4.5,1))
ax2.legend([u'$\mathrm{observed}$',u'$\mathrm{filtered},\ \omega_c=10^{-2}$',u'$\mathrm{expected}$'],frameon=False,loc=0,fontsize=12)
ax2.grid(True)
fig.tight_layout()
