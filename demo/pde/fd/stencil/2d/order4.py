...
Lf = f.diff(x,x,x,x) + f.diff(y,y,y,y)
...
ax[0][0].set_title('stencil size = 15')
ax[0][0].set_aspect('equal')
N = 15
L = rbf.fd.diff_matrix(nodes,[[4,0],[0,4]],size=N)
diff_est = L.dot(val)
...
# Stencil Size = 10
ax[0][1].set_title('stencil size = 20')
...
# Stencil Size = 20
ax[1][0].set_title('stencil size = 25')
...
# Stencil Size = 30
ax[1][1].set_title('stencil size = 30')
...
for i,s in enumerate(sizes):
  L = rbf.fd.diff_matrix(nodes,[[4,0],[0,4]],size=s)
  diff_est = L.dot(val)
  err = np.abs(diff_est - diff_true)
