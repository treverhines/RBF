...
ax[0][0].set_title('stencil size = 10')
ax[0][0].set_aspect('equal')
N = 10
L = rbf.fd.diff_matrix(nodes,[[3,0],[0,3]],size=N)
diff_est = L.dot(val)
...
# Stencil Size = 10
ax[0][1].set_title('stencil size = 15')
...
# Stencil Size = 20
ax[1][0].set_title('stencil size = 20')
...
# Stencil Size = 30
ax[1][1].set_title('stencil size = 25')
...
for i,s in enumerate(sizes):
  L = rbf.fd.diff_matrix(nodes,[[3,0],[0,3]],size=s)
  diff_est = L.dot(val)
  err = np.abs(diff_est - diff_true)
