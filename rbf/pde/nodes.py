...
def prepare_nodes(nodes, domain,
    ...
    # dispersion, but they do not move. TODO There is chance that one of the
    # points in `fixed_nodes` is equal to a point in `nodes`. This situation
    # should be handled
    fixed_nodes = np.zeros((0, domain.dim), dtype=float)
    ...
    # TODO: There should be a test to make sure each simplex belongs to at most
    # one group.
    ...
    for k, v in boundary_groups.items():
        # convert the list of simplices in the boundary group to a set, because
        # it is much faster to determine membership of a set
        v = set(v)
